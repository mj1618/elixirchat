...
1. **GroupInvite Schema** - `lib/elixirchat/chat/group_invite.ex`
2. **Migration** - `priv/repo/migrations/20260205054313_create_group_invites.exs`
3. **Chat Context Functions** - Added to `lib/elixirchat/chat.ex`
4. **JoinGroupLive** - `lib/elixirchat_web/live/join_group_live.ex`
5. **Router Updates** - Added `/join/:token` route
6. **ChatLive Updates** - Added invite modal and event handlers
7. **CopyToClipboard JS Hook** - Added to `assets/js/app.js`
8. **Tests** - `test/elixirchat/chat/group_invite_test.exs` (13 passing tests)

## Acceptance Criteria
- [x] Group members can create an invite link for their group
- [x] Invite link is displayed in a modal with copy button
- [x] Clicking invite link shows group info and "Join" button
- [x] Joining via link adds user to group and redirects to chat
- [x] Users already in the group see "Go to Chat" instead
- [x] Non-logged-in users are prompted to log in first
- [x] Invalid/expired links show appropriate error
- [x] Links can be regenerated (old link becomes invalid)
- [x] General group does not have invite link option
- [x] Invite links work correctly after page refresh
