...
## Completion Note (Agent ceefbe20)

Implemented reply-to-messages feature:

1. **Migration** (`priv/repo/migrations/20260205044854_add_reply_to_messages.exs`):
   - Added `reply_to_id` column with foreign key to messages table
   - Created index on `reply_to_id`

2. **Message Schema** (`lib/elixirchat/chat/message.ex`):
   - Added `belongs_to :reply_to, __MODULE__` association
   - Updated changeset to include `reply_to_id` with foreign key constraint

3. **Chat Context** (`lib/elixirchat/chat.ex`):
   - Updated `send_message/4` to accept optional `reply_to_id` via opts
   - Added validation that reply_to message exists in same conversation
   - Updated `list_messages/2` to preload `reply_to` with sender

4. **ChatLive** (`lib/elixirchat_web/live/chat_live.ex`):
   - Added `replying_to` to socket assigns
   - Added event handlers: `start_reply`, `cancel_reply`, `scroll_to_message`
   - Updated `send_message` to include reply_to_id when replying
   - Added reply button to message actions (next to reactions)
   - Added reply preview above message bubbles for replies
   - Added reply indicator above input showing what message is being replied to
   - Added `truncate/2` helper for text truncation

5. **Tests**: All 38 existing chat tests pass
