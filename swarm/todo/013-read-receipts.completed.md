...
1. Created `ReadReceipt` schema in `lib/elixirchat/chat/read_receipt.ex`
2. Created database migration `priv/repo/migrations/20260205045010_create_read_receipts.exs`
3. Added read receipt functions to Chat context:
   - `mark_messages_read/3` - Batch mark messages as read
   - `get_read_receipts_for_messages/1` - Get read status for messages
   - `get_message_readers/1` - Get users who have read a message
   - `message_read_by_other?/2` - Check if message is read by recipient
   - `broadcast_messages_read/3` - PubSub broadcast for real-time updates
4. Updated ChatLive to:
   - Load read receipts on mount
   - Handle `"messages_viewed"` event from JavaScript
   - Handle `{:messages_read, ...}` PubSub message
   - Display read receipt indicators (checkmarks for direct, "Read by X" for groups)
5. Added JavaScript hook in `assets/js/app.js`:
   - Uses IntersectionObserver to detect visible messages
   - Batches read events with 500ms debounce
   - Tracks which messages have been marked as read

### UI Features:
- Direct chats: Shows "Delivered" (single checkmark) or "Read" (double checkmark)
- Group chats: Shows "Read by X/Y" with tooltip showing reader names
- Real-time updates when other users read messages

### Files modified:
- `lib/elixirchat/chat/read_receipt.ex` (new)
- `lib/elixirchat/chat.ex`
- `lib/elixirchat_web/live/chat_live.ex`
- `assets/js/app.js`
- `priv/repo/migrations/20260205045010_create_read_receipts.exs` (new)
